...
class FileDB extends Database {
  ...
  async validateToken(token) {
    ...
    if(typeof this.dbInfo.tokens[token] === "undefined") {
  ...
  async getTokenByIdentifier(tokenChannelID) {
    if(typeof tokenChannelID === "undefined") {
      ...
      if(new Date().getTime() > Database.tokens[tokenChannelID].time.getTime()+2.16e+7) {
        ...
        delete this.dbInfo.tokens[token];
      }
      else {
        // Token already exists and is valid, return it and continue.
        return Database.tokens[tokenChannelID].token;
      }
    }
    ...
    crypto.randomBytes(32, (err, buffer) => {
      ...
      this.dbInfo.tokens[newToken] = { time: new Date(), random: [] };

      Database.tokens[tokenChannelID] = { token: newToken, time: this.dbInfo.tokens[newToken].time };
      return newToken;
    });
  }
  ...
  async resetToken(token) {
    return Object.assign({}, this.dbInfo.tokens[token] = { time: new Date(), random: [] });
  }
    ...
    for(var i = 0; i <= options.amount-1; i++) {
      ...
      try {
        ...
        if(typeof token !== "undefined") {
          ...
          if(isRandomCategory) {
            exclusionArray = JSON.parse(JSON.stringify(this.dbInfo.tokens[token].random || []));
          }
          else {
            exclusionArray = JSON.parse(JSON.stringify(this.dbInfo.tokens[token][category] || []));
          }
