...
module.exports = (input) => {
  ...
  Database.fetchQuestions = async (options) => {
    ...
    for(var i = 0; i <= options.amount-1; i++) {
      ...
      try {
        ...
        if(typeof token !== "undefined") {
          ...
          Trivia.debugLog(`Spliced ${exclusionArray.length} questions of ${length} with remainder ${exclusionPool.length} in ${erAfter-erBefore}ms`);
        ...
        if(typeof token !== "undefined") {
          if(isRandomCategory) {
            ...
            Trivia.debugLog("Pushing " + question.id + " to token array (rand).");
          }
          else {
            ...
            Trivia.debugLog("Pushing " + question.id + " to token array (" + category + ").");
          }
        }
      }
      catch(err) {
    ...
    var tAfter = Date.now();
    Trivia.debugLog(`File DB fetched ${questions.length} question(s) in ${tAfter-tBefore}ms.`);
