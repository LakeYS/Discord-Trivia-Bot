    ...
    for(var i = 0; i <= options.amount-1; i++) {
      ...
      try {
        ...
        if(typeof token !== "undefined") {
          ...
          var length = json.questions.length;
          for(var iB in exclusionArray) {
            var result = json.questions.splice(exclusionArray[iB], 1);
          ...
          console.log(`Spliced ${exclusionArray.length} questions of ${length} with remainder ${json.questions.length} in ${erAfter-erBefore}ms`);
