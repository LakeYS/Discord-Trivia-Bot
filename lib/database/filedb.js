      ...
      try {
        ...
        if(typeof token !== "undefined") {
          ...
          for(var iB in exclusionArray) {
            var result = exclusionPool.splice(exclusionArray[iB], 1);
            // If the splice doesn't echo the question back, something has gone horribly wrong.
            if(result.length === 0) {
              throw new Error(`Failed to process question pool at index ${iB}.`);
            }
          }
        ...
        var id = Math.floor(random * Object.values(exclusionPool).length);

        // Get the question using the random ID we just picked.
        question = Object.values(exclusionPool)[id];
