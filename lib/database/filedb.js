import { randomBytes as randomBytesOriginal } from "crypto";
...
var randomBytes = promisify(randomBytesOriginal);

export default class FileDB extends Database {
  ...
  parseCategories() {
    try {
      var categoryFile = load(readFileSync(this.path + "/categories.yml").toString());
  ...
  async updateGlobals() {
    ...
    try {
      ...
      for(i in this.dbInfo.categoryList) {
        ...
        questionPool = load(readFileSync(this.path + `/questions_${this.dbInfo.categoryList[i].id}.yml`).toString());
  ...
  async fetchQuestions(options) {
    ...
    for(var i = 0; i <= options.amount-1; i++) {
      ...
      try {
        var exclusionPool;
        if(!isRandomCategory) {
          exclusionPool = load(readFileSync(this.path + `/questions_${category}.yml`).toString()).questions;
        }
        else {
