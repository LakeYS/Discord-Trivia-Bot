    ...
    try {
      ...
      for(i in DatabaseInfo.categoryList) {
        ...
        for(var i2 in questionPool.questions) {
          ...
          if(typeof question.difficulty !== "string" || !config["score-value"].hasOwnProperty(question.difficulty)) {
  ...
  Database.fetchQuestions = async (options) => {
    ...
    for(var i = 0; i <= options.amount-1; i++) {
      ...
      try {
        var exclusionPool;
        if(!isRandomCategory) {
          exclusionPool = yaml.safeLoad(fs.readFileSync(path + `/questions_${category}.yml`).toString()).questons;
        }
        else {
        ...
        if(typeof token !== "undefined") {
          ...
          for(var iB in exclusionArray) {
            ...
            // "exclusion ID", or the index of the question ID as it appears in the token cache.
            // TODO: Optimize this so it does not need to run once for each entry in the exclusion array.
            var exclusionID = exclusionPool.findIndex((el) => {
              return el.id === exclusionArray[iB];
            });
        ...
        if(err.code !== 4) {
          console.log(`Failed to parse file 'questions_${category}.yml' with error '${err.message}'`);
