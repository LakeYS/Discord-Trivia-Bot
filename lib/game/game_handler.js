...
class GameHandler extends EventEmitter {
  ...
  createGame(reply, GameHandler, channelId, groupID, ownerID, options, gameMode) {
    if(this.getActiveGame(channelId)) {
      throw new Error("Game already exists in channel.");
    }

    if(gameMode === "hangman") {
      var game = new HangmanGame(reply, GameHandler, channelId, groupID, ownerID, options, gameMode);
      return game;
    }
    else {
      return new Game(reply, GameHandler, channelId, groupID, ownerID, options, gameMode);
    }
  }
}
