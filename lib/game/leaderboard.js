...
export default class Leaderboard {
  ...
  // participantsToDisplay can be game.usersTotal, or only correct participants for a much shorter list.
  makeScoreStr(scores, participantsToDisplay, largeMode, scoresBonus) {
    var scoreArray = [];
    var finalStr = "";

    for(var user in scores) {
      // Skip if this user is not in the participants list.
      if(typeof participantsToDisplay[user] !== "undefined") {
    ...
    scoreArray.forEach((userB) => {
      ...
      const scoreBonus = scoresBonus[userB];

      let strScoreNum;
      if (scoreBonus) {
        const strPreBonus = score-scoreBonus.toLocaleString();
        const strBonus = scoreBonus.toLocaleString();
        strScoreNum = `${strPreBonus} + bonus ${strBonus} pts`;
      } else {
        strScoreNum = `${score.toLocaleString()} pts`;
      }

      finalStr += `${finalStr!==""?"\n":""}`;
      const participantName = participantsToDisplay[userB];
      if(largeMode) {
        const userIndex = scoreArray.indexOf(userB)+1;
        finalStr += `${userIndex}. ${participantName} - ${strScoreNum}`;
      } else {
        finalStr += `${participantsToDisplay[userB]}: ${strScoreNum}`;
      }
    });
      ...
      for(var section in scores[guildId]) {
        ...
        if(typeof prop.expiredOn === "undefined") {
          ...
          if(section.endsWith("Monthly")) {
            if(typeof prop.expireDate === "undefined" || new Date(prop.expireDate).getDate() !== this.Trivia.getConfig("board-monthly-reset-day")) {
              dCurr = new Date();
              dExp = new Date();
              dExp.setMonth(dCurr.getMonth()+1);
              dExp.setDate(this.Trivia.getConfig("board-monthly-reset-day")); // Default: 1
              dExp.setMinutes(0);
              dExp.setHours(0);
              dExp.setSeconds(0);
          ...
          if(section.endsWith("Weekly")) {
            dCurr = new Date();
            var weekDay = dCurr.getDate() + (7 - dCurr.getDay()) + this.Trivia.getConfig("board-weekly-reset-day");
  ...
  readScores(guildId, section, includeProperties, doSectionSearch) {
    ...
    if(!existsSync("./Scores/scores.json")) {
      throw errEmpty;
    }

    var json = JSON.parse(readFileSync("./Scores/scores.json"));

    if(typeof json[guildId] === "undefined") {
      throw errEmpty;
    }

    json = this.refreshScores(json);
  ...
  writeScores(scores, guildId, sections, teamName) {
    if(!existsSync("./Scores/scores.json")) {
      this.initStatFolder();
    }

    let scoresOld; let json = {};
    if(existsSync("./Scores/scores.json")) {
      ...
      setTimeout(() => {
        copyFileSync("./Scores/scores.json", "./Scores/scores.json.bak");
      }, 1000);

      // We'll need to read the raw file in order to append to it.
      json = JSON.parse(readFileSync("./Scores/scores.json"));
      json = this.refreshScores(json);
    }
    ...
    if(typeof scoreData !== "object") {
      ...
      writeFile("./Scores/scores.json", saveData, "utf8", (err) => {
        if(err) {
