...
export default class GameStrings {
  ...
  buildEndStr(game, isEnding, hideAnswerOverride, gameMessage) {
    ...
    const hideAnswer = hideAnswerOverride || !game.getConfig("reveal-answers");
    ...
    if(!scoresDisabled) {
      ...
      if(!isEnding && Object.keys(game.usersCorrect).length === 0) {
        ...
        var scoreStr = game.gameHandler.leaderboard.makeScoreStr(
          game.scores, participantsToDisplay, false, game.scoresBonus
        );
