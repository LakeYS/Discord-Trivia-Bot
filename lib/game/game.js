...
import { TriviaInteraction } from "../platform/instance_common.js";

import GameStrings from "./game_strings.js";
...
export default class Game extends EventEmitter {
   ...
   *
   * @param {TriviaInteraction} interactionHelper
   */
  constructor(interactionHelper, GameHandler, channelId, groupID, ownerID, options, gameMode) {
    super();

    // Interaction helper for replying, etc.
    ...
    // Identifiers
    this.ID = channelId; // The game's unique identifier. Equivalent to the channel ID for now.
    ...
    this.scores = {};
    /**
     * Bonus score value (Reset per-round)
     * @type {Record<number, number>}
     */
    this.scoresBonus = {};
  ...
  buildAnswers() {
    ...
    for(var i = 0; i <= answers.length-1; i++) {
      ...
      answerString = `${answerString}**${String.fromCharCode(65+i)}:** ${decode(answers[i])}\n`;
    }
  ...
  async initializeRound() {
    ...
    var categoryString = decode(question.category);
    ...
    if(this.getConfig("debug-mode")) {
      infoString = `${infoString}\n*(Debug Mode - Answer: **${decode(question.correct_answer)}**)*`;
    }
  ...
  submitAnswer(userId, username, isCorrect) {
    ...
    if(this.getConfig("accept-first-answer-only") && typeof this.usersActive[userId] !== "undefined") {
      return;
    }
    ...
    this.scores[userId] ??= 0; this.scoresBonus[userId] ??= 0;
  ...
  async endRound(endStr) {
    ...
    // TODO this can probably be refactored using event handlers to separate
    // game extension code from core game code.
    this.gameHandler.extensions.applyUniqueBonus(this);
