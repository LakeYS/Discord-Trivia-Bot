...
class GameExtensions {
  ...
  sendRules(game) {
    let rules_string = "";
    if(game.getConfig("hangman-mode")) {
    ...
    else if(game.getConfig("use-reactions"))
    ...
    if(game.getConfig("accept-first-answer-only"))
      rules_string += "`\n - :one: Only your first answer counts. **No need to spam different answers**.`";

    rules_string += `\n - :hourglass_flowing_sand: Each round will last ${game.getConfig("round-length")/1000} seconds.`;
  ...
  getUserUniqueBonus(game, userID) {
    ...
    if(multiplierBase !== 0) {
      ...
      if(multiplier > 1) {
        ...
        game.debugLog(`Applied bonus score of ${bonus} to user ${userID}`);
        return bonus;
      }
    }
  }
  ...
  applyUniqueBonus(game) {
    for(var userID in game.usersActive) {
      var bonus = this.getUserBonus(game, userID);

      this.scores[userID] += bonus;
    }
  }
}
