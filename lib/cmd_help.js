...
module.exports = (config, Trivia) => {
  ...
  return async function(msg, Database) {
    ...
    var footerTemplate = `Total questions: ${apiCountGlobal.toLocaleString()}`;
    ...
    const body = `Let's play trivia! Type \`${config.prefix}play\` to start a game.${stringContribute}\n\nBot by [Lake Y](http://lakeys.net). Powered by discord.js ${pjson.dependencies["discord.js"].replace("^","")}` +
