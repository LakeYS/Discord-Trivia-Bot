module.exports = (config) => {
  ...
  const footerString = `Commands: \`${config.prefix}play <category>\`, \`${config.prefix}help\`, \`${config.prefix}categories\`, \`${config.prefix}stop\`, \`${config.prefix}ping\`${config["league-commands"]?`\nLeague Commands: \`${config.prefix}league stats\`, \`${config.prefix}league rank <name or tag>\``:""}
