module.exports = (config) => {
  ...
  const footerString = `Commands: \`${config.prefix}play <category>\`, \`${config.prefix}play advanced\`, \`${config.prefix}help\`, \`${config.prefix}categories\`, \`${config.prefix}stop <#channel>\`, \`${config.prefix}ping\`${config["league-commands"]?`\nType '${config.prefix}league help' for league commands.`:""}
