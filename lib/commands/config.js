module.exports = (Trivia, ConfigData, Config) => {
  ...
  return function(cmd, channel, user, isAdmin) {
    if(isAdmin && Trivia.getConfigVal("config-commands-enabled")) {
      var cmdInput = cmd.replace("CONFIG ","");
  
      if(cmdInput === "CONFIG") {
        Trivia.send(channel, void 0, `Must specify an option to configure. \`${Trivia.getConfigVal("prefix")}config <option> <value>\``);
        return;
      }
      ...
      if(cmdInput.startsWith("LIST") && cmdInput.indexOf("-") === -1) {
        ...
        var configStr = `**__Config Options__**\nThese are the config options that are currently loaded${typeof listID!=="undefined"?` in the channel <#${listID}>`:""}. Some options require a restart to take effect. Type '${Trivia.getConfigVal("prefix")}reset' to apply changes.`;
        ...
        for(var i in Config) {
          if(i.toString().includes("token") || i.toString().includes("comment") || i.includes("configFile")) {
            ...
            var value = Trivia.getConfigVal(i, listID);
        ...
        if(configVal === `CONFIG ${configKey}`) {
          Trivia.send(channel, void 0, `Must specify a value. \`${Trivia.getConfigVal("prefix")}config <option> <value>\``);
          return;
        }
        ...
        if(configVal === Trivia.getConfigVal(configKey.toLowerCase(), channel)) {
