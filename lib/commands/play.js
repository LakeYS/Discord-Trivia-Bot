    ...
    if(categoryInput !== "PLAY" && categoryInput !== "PLAY HANGMAN") {
      ...
      .then((category) => {
        if(typeof category === "undefined") {
          ...
          Trivia.gameHandler.createGame(Trivia.gameHandler, msg.channel.id, guildId, msg.author.id, questionOptions, mode).initializeRound();
          return;
        }
      })
      .catch((err) => {
      ...
      Trivia.gameHandler.createGame(Trivia.gameHandler, msg.channel.id, guildId, msg.author.id, {allowLongAnswers: Trivia.database.allowLongAnswers}, mode).initializeRound();
      return;
    }
  };
};
