module.exports = (config, Trivia, getConfigVal) => {
  return function (msg, categoryInput, mode) {
    ...
    if(categoryInput !== "PLAY" && categoryInput !== "PLAY HANGMAN") {
      ...
      Trivia.gameHandler.createGame(Trivia.gameHandler, msg.channel.id, msg.channel.guild.id, msg.author.id, {}, mode).initializeRound();
      return;
    }
  };
};
