  ...
  return function(reply, replyDirect, game, isAdmin) {
    if(typeof game !== "undefined" && game.inProgress) {
      ...
      if(isAdmin) {
        ...
        reply({content: `Trivia games will end automatically if the game is inactive for more than ${getConfig("rounds-end-after")} round${getConfig("rounds-end-after")===1?"":"s"}. Only users with the "Manage Server" permission can force-end a game.` });
      }
