  ...
  return function(msg, cmd, isAdmin) {
    ...
    if(isAdmin) {
      ...
      if(channelInput !== "STOP") {
        ...
        game = Trivia.gameHandler.getActiveGame(stopChannel);
    ...
    if(typeof game !== "undefined" && game.inProgress) {
      if(isAdmin) {
        ...
        Trivia.send(msg.channel, void 0, `Trivia games will end automatically if the game is inactive for more than ${Trivia.getConfig("rounds-end-after", msg.channel)-1} round${Trivia.getConfig("rounds-end-after", msg.channel)-1===1?"":"s"}. Only users with the "Manage Server" permission can force-end a game.`);
      }
