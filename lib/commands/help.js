...
import { TriviaInstance } from "../platform/instance_common.js";
 ...
 */
export default (config, Trivia, client) => {

  return async function(interactionHelper, Database) {
    ...
    if(useSlashCommands) {
      interactionHelper.maybeReply({embeds: [embed], components}, {direct: true});
    } else {
      interactionHelper.maybeReply({embeds: [embed], components});
    }
  };
};
