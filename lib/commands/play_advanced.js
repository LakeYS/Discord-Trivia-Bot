const { ChannelType, CommandInteractionOptionResolver, User } = require("discord.js");
const { TriviaInteractionDiscord } = require("../platform/discord_main");
...
module.exports = (Trivia) => {
  ...
  function getAdvStr(identifier, data) {
    switch(identifier) {
      case "mode":
        ...
        1. Normal: *A standard four-letter trivia game.*

        2. Typed answer (classic): *Classic mode. Type your answers instead of using buttons.*

        3. Reaction mode: *Use reactions to keep score!*

        4. Hangman: *Hangman-styled 'guess the answer' game. #IRC*
  ...
  async function startAdvGame(id, author) {
    ...
    const interactionHelper = advData[id].interactionHelper;
    ...
    // User ID or guild ID
    const groupID = startChannelB.type === ChannelType.DM ? startChannelB.id : startChannelB.guild.id;

    var game = Trivia.gameHandler.createGame(interactionHelper, Trivia.gameHandler, startChannelB.id, groupID, author.id, question, mode).initializeRound();
  ...
  advActions.mode = (id, cmd) => {
    ...
    const modeNumDisplay = parseInt(cmd);
    advData[id].pendingMode = false;
    advData[id].pendingCategory = true;

    advData[id].argStr = `${advData[id].argStr} ${modeNumDisplay}`;
  ...
  advActions.channel = (id, cmd, channel) => {
    var startChannel;
    var isDMChannel = channel.type === ChannelType.DM;
    ...
    if(Trivia.gameHandler.getActiveGame(startChannel.id) !== undefined) {
      return {
        error: true,
        text: "There is already a game active in that channel."
      };
    }
  ...
  /**
   * Parse a set of args for custom game setup via Discord.js interaction.
   *
   * @param {import("discord.js").Channel} channel 
   * @param {User} author 
   * @param {TriviaInteractionDiscord} interactionHelper The interaction helper. 
   * @param {CommandInteractionOptionResolver} argsObject
   * @returns 
   */
  async function parseAdvCommandArgs(channel, author, interactionHelper, argsObject) {
    // TODO Respond with an error if the user selected true/false in hangman
    // TODO parse "any" (with any case) under category for any category
    ...
    const categoryRaw = argsObject.getString("category") ?? "all";

    const categoryInput = categoryRaw.toUpperCase() === "ALL" ? "RANDOM" : categoryRaw;

    const channelObj = argsObject.getChannel("channel");
    const channelResult = channelObj == null ? "HERE" : channelObj.id;

    const data = {
      mode: modeIndex[argsObject.getString("gamemode")],
      categoryInput,
      type: typeIndex[argsObject.getString("type") ?? "all"],
      difficulty: difficultyIndex[argsObject.getString("difficulty") ?? "all"],
    ...
    for(var action in advActions) {
      ...
      try {
        if(action === "category") {
          result = await advActions[action](channel.id, data.categoryInput);
        }
        else {
          result = await advActions[action](channel.id, data[action], channel);

          if(result.error) {
            // This should only ever happen with a category error.
        ...
        console.log(`An error occurred while parsing advanced game args from a slash command:\n${err.stack}`);
      }
    }

    // Reply direct only in slash command config
    interactionHelper.maybeReply(
      `Starting game in <#${advData[channel.id].startChannel.id}>...`,
      {direct: getConfig("use-slash-commands"), noAutoDelete: true}
    );
  ...
  /**
   * Parse a set of args for game setup via plaintext/text chat command.
   *
   * @param {TriviaInteractionDiscord} interactionHelper The interaction helper. 
   * @returns {Promise<void>}
   */
  async function parseAdvTextArgs(channel, author, interactionHelper, cmd) {
    ...
    for(var action in advActions) {
      ...
      try {
        if(action === "category") {
          ...
          if(result.error) {
            interactionHelper.maybeReply("Invalid input, please try again.", {direct: true});
  ...
  /**
   * Command entrypoint for game setup.
   *
   * @param {TriviaInteractionDiscord} interactionHelper The interaction helper.
   * @returns {void}
   */
  function triviaPlayAdvanced(interactionHelper, onGameStart, id, channel, author, cmd, customArgStr, config, argsObject) {
    // Initialize an empty config object if there is none.
    config = config || {};

    if(typeof advData[id] === "undefined") {

      advData[id] = { user: author.id, pendingMode: true, onGameStart, config, interactionHelper };
      ...
      if(argsObject != null) {
        // Slash commands
        parseAdvCommandArgs(channel, author, interactionHelper, argsObject);
        return;
      } else if(cmd !== "" && typeof cmd !== "undefined") {
        // Text commands
        parseAdvTextArgs(channel, author, interactionHelper, cmd.replace(" ", ""));
        return;
      }
      ...
      .then((sent) => {
        // Message failed to send silently -- cancel the setup
        if(sent == null) {
          cancelAdvGame(id);
          return;
        }
        ...
        if(!getConfig("debug-mode")) {
          advData[id].timeout = setTimeout(() => {
            cancelAdvGame(id);
            interactionHelper.maybeReply({content: "Advanced game cancelled due to inactivity."});
          }, 25000);
        }
      });
    }
  ...
  async function doAdvAction(interactionHelper, id, msg, cmd, userIsAdmin) {
    var userIsStarter = msg.author.id === advData[id].user;

    if(cmd === "CANCEL" && (userIsStarter || userIsAdmin)) {
      cancelAdvGame(id);
      interactionHelper.maybeReply(
        {content: "Game setup cancelled."}, {noAutoDelete: true}
      );
      return;
    }
    ...
    interactionHelper.maybeReply({embed: {
      ...
      footer: {
        text: "To repeat this configuration, use the command: " +
        advData[id].argStr
      }
    }}, {noAutoDelete: true});
  ...
  async function parseAdv(id, msg, isAdmin) {
    ...
    doAdvAction(interactionHelper, id, msg, cmd, isAdmin)
    .catch((err) => {
      ...
      if(typeof advData[id] !== "undefined") {
        ...
        interactionHelper.maybeReply({embed: {
          color: 14164000,
          description: "An internal error occurred while parsing advanced game data."
        }});
      }
    })
    .then((response) => {
