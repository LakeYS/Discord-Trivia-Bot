...
module.exports = (Trivia) => {
  ...
  async function startAdvGame(id, author) {
    ...
    var game = Trivia.gameHandler.createGame(reply, Trivia.gameHandler, startChannelB.id, groupID, author.id, question, mode).initializeRound();
  ...
  async function parseAdvCommandArgs(channel, author, reply, replyDirect, argsObject) {
    ...
    replyDirect(`Starting game in <#${advData[channel.id].startChannel.id}>...`, void 0, true);
  ...
  async function parseAdvTextArgs(channel, author, reply, replyDirect, cmd) {
    ...
    for(var action in advActions) {
      ...
      try {
        if(action === "category") {
          ...
          if(result.error) {
            replyDirect("Invalid input, please try again.");
    ...
    reply(`Starting game in <#${advData[channel.id].startChannel.id}>...`, void 0, true);
  ...
  function triviaPlayAdvanced(reply, replyDirect, onGameStart, id, channel, author, cmd, customArgStr, config, argsObject) {
    // Initialize an empty config object if there is none.
    config = config || {};

    if(typeof advData[id] === "undefined") {

      advData[id] = { user: author.id, pendingMode: true, onGameStart, config, reply };
      ...
      if(argsObject != null) {
        // Slash commands
        parseAdvCommandArgs(channel, author, reply, replyDirect, argsObject);
        return;
      } else if(cmd !== "" && typeof cmd !== "undefined") {
        // Text commands
        parseAdvTextArgs(channel, author, reply, replyDirect, cmd.replace(" ", ""));
        return;
      }
      ...
      .then((sent) => {
        ...
        if(!getConfig("debug-mode")) {
          advData[id].timeout = setTimeout(() => {
            cancelAdvGame(id);
            reply({content: "Advanced game cancelled due to inactivity."});
          }, 25000);
        }
      });
    }
  }
  async function doAdvAction(reply, id, msg, cmd, userIsAdmin) {
    var userIsStarter = msg.author.id === advData[id].user;

    if(cmd === "CANCEL" && (userIsStarter || userIsAdmin)) {
      cancelAdvGame(id);
      reply({content: "Game setup cancelled."}, void 0, true);
      return;
    }
  ...
  async function parseAdv(id, msg, isAdmin) {
    ...
    doAdvAction(reply, id, msg, cmd, isAdmin)
    .catch((err) => {
