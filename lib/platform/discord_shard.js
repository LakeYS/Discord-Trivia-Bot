...
import DiscordCommands from "./discord_commands.js";
import { TriviaDiscord } from "./discord_main.js";

var configData = configFunc(process.argv[2]);
...
var Trivia = client.Trivia = new TriviaDiscord(client, configData);
...
client.on("clientReady", async () => {
  ...
  Trivia.discordCommandHandler = new DiscordCommands(client, client.token, config["debug-slash-commands-guild"]);
...
client.on("messageCreate", async (msg) => {  
  try {
    ...
    if(msg.channel.type !== ChannelType.GuildText && msg.channel.type !== ChannelType.DM) return;

    Trivia.parseText(str, msg);
  } catch(err) {
    console.error("Error during text handling:");
    console.error(err);
  }
});
...
client.on("messageReactionAdd", (reaction, user) => {
  try {
    ...
    console.error("Error during reaction handling:");
    console.error(err);
  }
});
...
async function onButtonPress(interaction) {
  ...
  if(interaction.customId.startsWith("answer_")) {
    ...
    var participants = await Trivia.buttonPress(interaction, answer, interaction.user.id, name);
    ...
    if(participants === -1) {
      ...
      if(now.getTime() < interaction.message.createdAt.getTime()+60000) {
        ...
        console.warn(`Received a late response ${offTime}ms after round end. Ignoring and sending "This round has already ended" to user. Source: ${interaction.user.id}`);
      }
...
client.on("interactionCreate", async interaction => {
  try {
    if (interaction.isButton()) {
      ...
      await Trivia.parseSlash(interaction.commandName, interaction);
    }
  } catch(err) {
...
client.on("guildCreate", () => {
  try {
    ...
    console.error("Error handling guild create event:");
    console.error(err);
  }
});
...
client.on("guildDelete", () => {
  try {
    ...
    console.error("Error handling guild delete event:");
    console.error(err);
  }
});
