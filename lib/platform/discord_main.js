...
import CmdCategories from "../commands/categories.js";
import CmdHelp from "../commands/help.js";
import CmdPing from "../commands/ping.js";
import CmdPlay from "../commands/play.js";
import CmdPlayAdvanced from "../commands/play_advanced.js";
import CmdStop from "../commands/stop.js";
import FileDB from "../database/filedb.js";
import MergerDB from "../database/mergerdb.js";
import OpenTDB from "../database/opentdb.js";
import Game from "../game/game.js";
import GameDebugger from "../game/game_debug.js";
import GameHandler from "../game/game_handler.js";
import Listings from "../listings_discord.js";

import {
  ReplyProperties, TriviaInstance, TriviaInteraction
} from "./instance_common.js";
...
export class TriviaDiscord extends TriviaInstance {
  /**
   * An index of channels used for each game, if applicable.
   * @type {{gameChannels: Record<number, TextChannel>}}
   */
  discord = {
    gameChannels: {}
  };
  ...
  constructor(client, configData) {
    ...
    var playAdv = CmdPlayAdvanced(this, client);

    const prefixStr = this.getConfig("use-slash-commands") ? "/" : this.getConfig("prefix");

    this.commands = {
      playAdv,
      triviaHelp: CmdHelp(this.config, this, client),
      triviaCategories: CmdCategories(this.config),
      triviaPlay: CmdPlay(this.config, this, prefixStr),
      triviaPlayAdvanced: playAdv.triviaPlayAdvanced,
      triviaStop: CmdStop(),
      triviaPing: CmdPing(this)
    };
  ...
  /**
   * Set up events, etc. for a new game.
   * @param {Game} game 
   */
  async handleGameCreate(game) {
      ...
      game.on("game_error", (err) => {
        if(err.code === 5) {
          if (game.getConfig("database-rate-limit-mode") === true) {
            console.warn("!!! Got rate limit code!");
            console.trace();
          }

          interactionHelper.maybeReply({embed: {
            color: 14164000,
            description: "Too many commands have been used at once. Please" +
        ...
        interactionHelper.maybeReply({embed: {
          color: 14164000,
          description: "An internal error has occurred. The database may " +
          "be inaccessible."
        }});
      });
      ...
      game.on("round_initialize", async (finalString) => {
        ...
        // Cancel if the message fails to send. The reply callback and/or error
        // catch will log if something went notably wrong aside from perms, etc
        // TODO improve error handling here to be more concise?
        if(msg == null) {
          game.endGame();
          return;
        }
  ...
  initGameHandler() {
    ...
    // GameHandler init
    this.gameHandler.on("game_create", (game) => this.handleGameCreate(game));
  ...
  getConfig(value, channel, guild) {
    ...
    if(typeof channel !== "undefined" && existsSync(file)) {
      ...
      if(this.configData.localOptions.includes(value)) {
        var currentConfig;
        try {
          currentConfig = readFileSync(file).toString();
        ...
        if(error.code === 4 && typeof token !== "undefined") {
          ...
          if(tokenRetry !== 1) {
            try {
              ...
              console.warn("Failed to reset token due to an internal error.");
              throw error;
            }
            ...
            if(isFirstQuestion) {
              ...
              throw new Error("Token reset loop.", { cause: error });
            }
          }
        }
        else {
   ...
   * @param {import("discord.js").Channel} channel - The channel we are operating
   * in.
   ...
   */
  async parseSlash(commandName, interaction) {
    ...
    // TODO This is an ugly hack for deferring commands for high-traffic use.
    // This falls apart if ephemeral messages are used, thus it is dependent
    // on these hardcoded commands not using ephemeral replies.
    /**
     * @type {CommandInteraction<import("discord.js").CacheType>["reply"] |
     * CommandInteraction<import("discord.js").CacheType>["editReply"]
     * }
     */
    let reply = interaction.reply.bind(interaction);
    const cmd = interaction.commandName;

    if (this.getConfig("database-rate-limit-mode") === true) {
      if(cmd === "setup" || cmd === "hangman" || cmd === "play") {
    ...
    const replyCallbackDirect = async (response) => {
      ...
      try {
        ...
        return await reply(modifiedResponse);
      } catch(err) {
     ...
     * response.
     * @returns {Promise<Message>} The message (simple passthrough of the
     * response from discord.js)
     */
    const replyCallback = async (response, props) => {
        ...
        if(error.message.includes("Missing Permissions")) {
          ...
          try {
            ...
            if(!interactionReplySent && !interaction.replied) {
              await replyCallbackDirect(message);
            }
            else {
      ...
      if(!hasChannelPerms) {
        await handleError(new Error("Missing Permissions for TriviaBot"));
        return null;
      }
      ...
      if(!interactionReplySent && !interaction.replied) {
        ...
        try {
          ...
          const res = await reply({...replyData, withResponse: true});
  ...
  parseText(str, msg) {
    ...
    const isAdmin = msg.member && this.checkAdmin(msg.channel, msg.member);
    ...
    const replyCallbackDirect = async (response) => {
      await this.send(msg.author, null, response);
    };
   ...
   *
   * @param {TextChannel | DMChannel | User} channel The channel to send to.
   * @param {User} [author] Author (Omit to prevent error messages from going to the author's DMs)
   * @param {string | any} msg Message Object. TODO more explicit type
  ...
  async send(channel, author, msg, callback, neverDelete) {
    let msgResult;
    try {
      msgResult = await channel.send(parseMsg(msg));
    } catch(err) {
    ...
    this.emit("message_sent", msgResult, { neverDelete });
  ...
  async updateGameButtons(game) {
    if(typeof game.buttons !== "undefined") {
      ...
      for(let i in game.buttons.components) {
        ...
        if(game.getConfig("reveal-answers") && parseInt(i) === game.question.displayCorrectID) {
      ...
      try {
        ...
        console.warn(err);
        console.warn(`Failed to edit game message in channel ${game.ID} (see` +
          " above error). Message may have been deleted. This will be " +
          "ignored.");
      }
  ...
  async buttonPress(interaction, answer, userId, username) {
    ...
    if(game.getConfig("accept-first-answer-only", interaction.channel.id) && typeof game.usersActive[userId] !== "undefined") {
      const msg = await interaction.reply({content: "You have already answered this question.", flags: ["Ephemeral"]});
   ...
   *
   * @param {import("discord.js").Channel} interactionChannel The channel that generated this
   ...
   *
   * @returns {import("discord.js").Channel} The channel object to use.
   */
...
export class TriviaInteractionDiscord extends TriviaInteraction {
  /**
