...
class TriviaDiscord extends TriviaInstance {
  ...
  initGameHandler() {
    ...
    this.gameHandler.on("game_create", async (game) => {
      ...
      game.on("round_end", async (endInfo) => {
        ...
        var doMessage = endInfo.str !== "" && !endInfo.gameIsEnding;
  ...
  parseAnswer(game, str, channelId, userId, username) {
    ...
    if(game.getConfig("disallow-answer-changes", channelId) && typeof game.usersActive[userId] !== "undefined") {
      return;
    }
  ...
  buttonPress(message, answer, userId, username) {
    ...
    this.parseAnswer(game, answer, id, userId, username,this.getConfig("score-value", message.channel));
