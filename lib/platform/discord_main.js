...
function setConfigVal(value, newValue, skipOverride, localID) {
  ...
  if(skipOverride !== true || !getConfigVal("config_commands_enabled")) {
...
global.client.on("ready", () => {
  ...
  var restrictedChannelsInput = getConfigVal("channel_whitelist");
...
Trivia.embedCol = Buffer.from(getConfigVal("embed_color").padStart(8, "0"), "hex").readInt32BE(0);
...
var allowLongAnswers = getConfigVal("database_allow_long_answers") || getConfigVal("hangman_mode");

if(getConfigVal("database_merge")) {
...
Trivia.isFallbackMode = (channel) => {
  if(getConfigVal("fallback_mode")) {
    if(typeof getConfigVal("fallback_exceptions") !== "undefined" && getConfigVal("fallback_exceptions").indexOf(channel) !== -1) {
...
Trivia.getTriviaQuestion = async function(initial, tokenChannelID, tokenRetry, isFirstQuestion, category, typeInput, difficultyInput) {
  ...
  if(typeof length === "undefined" || length < 2 || isCustom) {
    ...
    if(isCustom || Config.databaseURL.startsWith("file://")) {
      ...
      options.amount = getConfigVal("database_cache_size");
    }
    ...
    try {
      ...
      if(!tokenRetry && typeof token !== "undefined") {
...
Trivia.parseAnswer = function (game, str, channelId, userId, username) {
  ...
  if(getConfigVal("accept_first_answer_only", channelId) && typeof game.usersActive[userId] !== "undefined") {
    return;
  }
...
commands.playAdv = require("../commands/play_advanced.js")(Trivia, global.client);
var parseAdv = commands.playAdv.parseAdv;
commands.triviaHelp = require("../commands/help.js")(Config, Trivia);
commands.triviaCategories = require("../commands/categories.js")(Config);
commands.triviaPlay = require("../commands/play.js")(Config, Trivia, commands, getConfigVal);
commands.triviaPlayAdvanced = commands.playAdv.triviaPlayAdvanced;
commands.triviaStop = require("../commands/stop.js")(Config, Trivia, commands, getConfigVal);
commands.triviaConfig = require("../commands/config.js")(Trivia, ConfigData, Config);
commands.triviaPing = require("../commands/ping.js")(Trivia);
...
function parseCommand(msg, cmd, isAdmin) {
  ...
  if(cmd.startsWith("RESET")) {
    if(isAdmin && getConfigVal("config_commands_enabled")) {
...
Trivia.parse = (str, msg) => {
  ...
  if(msg.author.bot === true && getConfigVal("allow_bots") !== true) {
    return;
  }
...
Trivia.postStats = async () => {
  ...
  for(var site in Config.listing_tokens) {
    listings.setToken(site, Config.listing_tokens[site]);
  }
