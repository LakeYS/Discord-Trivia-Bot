...
class TriviaDiscord extends TriviaInstance {
    ...
    async (game) => {
      ...
      game.on("round_initialize", async (finalString) => {
        ...
        try {
          ...
          ], components}, {discord: {authorOverride}});
  ...
  parseSlash(commandName, interaction) {
     ...
     */
    const replyCallback = async (response) => {
  ...
  parseText(str, msg) {
    ...
    const replyCallback = (response, authorOverride) => {
   ...
   *
   * @param {GuildChannel | DMChannel} channel The channel to send to.
...
class TriviaInteractionDiscord extends TriviaInteraction {
  ...
  maybeReply(response, props) {
    if (props?.direct) {
      ...
      return this.replyCallback(response);
    }
  }
}
