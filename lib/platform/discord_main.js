...
class TriviaDiscord extends TriviaInstance {
  ...
  doCommand(cmd, reply, replyDirect, channel, member, msg) {
    ...
    if(cmd.startsWith("PLAY HANGMAN ") || cmd === "PLAY HANGMAN") {
      ...
      if(this.getConfig("databaseURL") === "https://opentdb.com") {
        this.send(channel, member.user, "*(Beware: Some questions from OpenTDB are not designed for hangman-style gameplay)*");
      }
  ...
  parseSlash(commandName, interaction) {
    ...
    this.doCommand(interaction.commandName, reply, replyDirect, interaction.channel, interaction.member);
  }
  ...
  parseText(str, msg) {
    ...
    const reply = (response) => {
      this.send(msg.channel, msg.author, response);
    };
    ...
    if(str.startsWith(prefix)) {
      ...
      this.doCommand(cmd, reply, replyDirect, msg.channel, msg.member, msg);
    }
  }
