...
class TriviaDiscord extends TriviaInstance {
  ...
  async send(channel, author, msg, callback, noDelete) {
    try {
      ...
      if(typeof author === "undefined") {
        console.warn("Failed to send message to channel, user object nonexistent. Err: " + err.message);
        console.trace();
        console.log(msg);
        return;
      }
  ...
  buildButtons(answers) {
    ...
    for(var i = 0; i <= answers.length-1; i++) {
      ...
      text = `${Letters[i]}${this.getConfig("database-allow-long-answers")?'':': '+this.formatStr(answers[i])}`;
