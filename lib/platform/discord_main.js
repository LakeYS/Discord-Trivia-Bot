...
const fs = require("fs");
const TriviaPlatform = require("./platform_common.js");
...
class TriviaDiscord extends TriviaPlatform {
  constructor(client, configData) {
    ...
    if(this.getConfigVal("database-merge")) {
      ...
      this.database = new MergerDB(this.config.databaseURL, allowLongAnswers);
    }
    else {
      // Check database protocol
      if(this.config.databaseURL.startsWith("file://")) {
        this.database = new FileDB(this.config.databaseURL, allowLongAnswers);
      }
      else {
        this.database = new OpenTDB(this.config.databaseURL);
      }
    }
    ...
    this.commands = {
      ...
      triviaPlay: require("../commands/play.js")(this.config, this, getConfigVal),
      triviaPlayAdvanced: playAdv.triviaPlayAdvanced,
      triviaStop: require("../commands/stop.js")(this.config, this, getConfigVal),
    ...
    this.gameHandler.on("game_create", async (game) => {
      ...
      game.on("round_initialize", async (finalString) => {
        ...
        if(game.getConfig("display-pregame-rules") && game.roundCount === 0) {
          ...
          this.send(channel, void 0, rules);
      ...
      game.on("round_end", async (endInfo) => {
        ...
        if(doMessage) {
          try {
            await this.send(channel, void 0, {embed: {
              color: game.color,
 ...
 getConfigVal(value, channel, guild) {
  ...
  parseCommand(msg, cmd, isAdmin) {
    ...
    if(cmd.startsWith("PLAY ") || cmd === "PLAY") {
      ...
      if(this.getConfigVal("hangman-mode")) {
        mode = "hangman";
      }
      else if(this.getConfigVal("use-reactions")) {
        mode = "reaction";
      }
