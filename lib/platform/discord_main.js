...
class TriviaDiscord extends TriviaInstance {
  ...
  doCommand(cmd, reply, replyDirect, channel, member, guild, msg) {
  ...
  parseSlash(commandName, interaction) {
    ...
    const reply = async (response) => {
      ...
      if(!interactionReplySent) {
        ...
        const interactionResponse = await interaction.channel.send(msg);
    ...
    this.doCommand(interaction.commandName, reply, replyDirect, interaction.channel, interaction.member, interaction.guild);
  }
