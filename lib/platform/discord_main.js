...
class TriviaDiscord extends TriviaInstance {
  ...
  doCommand(cmd, reply, replyDirect, channel, member, guild, msg, args) {
    ...
    if(cmd.startsWith("STOP")) {
      if(msg != null) {
        ...
        // Slash commands
        stopChannel = args !== null ? channel : args.getChannel("channel");
        const stopGame = this.gameHandler.getActiveGame(stopChannel.id);
        commands.triviaStop(reply, replyDirect, stopGame, isAdmin);
      }
    }
    ...
    if(cmd.startsWith("PLAY ADVANCED")) {
      ...
      commands.triviaPlayAdvanced(void 0, channel.id, channel, member.user, cmd.replace("PLAY ADVANCED",""));
      return;
    }
    ...
    if(cmd.startsWith("PLAY HANGMAN ") || cmd === "PLAY HANGMAN" || (msg === null && cmd === "HANGMAN")) {
      ...
      if(this.getConfig("databaseURL") === "https://opentdb.com") {
        game.reply("*(Beware: Some questions from OpenTDB are not designed for hangman-style gameplay)*");
      }
      
      commands.triviaPlay(reply, channel.id, member.user.id, guild, categoryInput, "hangman");
      this.postStat("commandPlayHangmanCount", 1);
      return;
    }

    if(cmd === "HELP") {
      commands.triviaHelp(reply, this.database);
      return;
    }
    ...
    if(cmd.startsWith("PLAY ") || cmd === "PLAY") {
      ...
      commands.triviaPlay(reply, channel.id, member.user.id, guild, categoryInput, mode);
      return;
    }
    ...
    const reply = async (response) => {
      ...
      const handleError = (error) => {
        if(error.message.includes("Missing Permissions")) {
          ...
          + " The permissions required are \"Send Messages\" and \"Embed Links\".";

          try {
            // Use an ephemeral message if possible. Otherwise, DM the initiator of the command.
            if(!interactionReplySent) {
              replyDirect(message);
            }
            else {
      ...
      const hasChannelPerms = interaction.appPermissions.has(
      ...
      if(!interactionReplySent) {
        ...
        try {
          interactionResponse = await interaction.reply(msg);
        }
        catch(err) {
  ...
  parseText(str, msg) {
    ...
    const reply = (response, authorOverride) => {
      ...
      this.send(msg.channel, author, parseMsg(response));
    };
    ...
    if(str === prefix + "HELP" || str.includes(`<@!${this.client.user.id}>`)) {
      this.commands.triviaHelp(reply, this.database);
      return;
    }
