...
module.exports = (Trivia) => {
  ...
  async function parseAdvArgs(channel, author, cmd) {
    ...
    try {
      await advActions.mode(args[0]);
      await advActions.category(channel.id, category);
      await advActions.type(channel.id, args[1]);
      await advActions.difficulty(channel.id, args[2]);
      await advActions.channel(channel.id, args[3], channel);
    } catch(err) {
      Trivia.send(channel, void 0, {embed: {
        color: 14164000,
        description: `An error occurred while parsing advanced game args:\n${err}`
      }});
    ...
    Trivia.send(channel, void 0, {embed: {
      ...
      description: `Starting game in channel <#${advData[channel.id].startChannel.id}>...`
    }}, void 0, true);
  ...
  async function doAdvAction(id, msg, cmd) {
    ...
    if(advData[id].pendingMode) {
      ...
      advActions.channel(id, cmd, msg.channel);
    }

    Trivia.send(msg.channel, void 0, {embed: {
      color: Trivia.embedCol,
      description: `Starting game in channel <#${advData[id].startChannel.id}>...`,
