...
module.exports = (Trivia, triviaSend, doTriviaGame, game, Database, embedCol) => {

  var getConfigVal = Trivia.getConfigVal;

  game, Database, embedCol;

  function triviaPlayAdvanced(id, channel, author) {
    if(getConfigVal("databaseURL").startsWith("file://")) {
      triviaSend(channel, author, "WIP - File databases are currently not supported with this command.", void 0, true);
  ...
  async function doAdvAction(id, msg) {
    if(advData[id]) {
      ...
      doTriviaGame(msg.channel.id, msg.channel, msg.author, 0, advData[id].categoryId, advData[id].type, advData[id].difficulty);
      delete advData[id];
    }
    else {
      return;
