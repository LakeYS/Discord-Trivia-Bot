...
module.exports = (Trivia) => {
  ...
  async function startAdvGame(id, author) {
    ...
    var game = await Trivia.doGame(
      advData[id].startChannel.id,
      advData[id].startChannel,
      author, 0, advData[id].categoryId,
      advData[id].type,
      advData[id].difficulty,
      advData[id].mode
    );
  ...
  function triviaPlayAdvanced(onGameStart, id, channel, author, cmd) {
    if(cmd !== "PLAY ADVANCED") {
      console.log(`Advanced game input: ${cmd}`)
    }
    if(typeof advData[id] === "undefined") {
