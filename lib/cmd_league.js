module.exports = (Trivia) => {
  ...
  async function leagueParse(id, channel, author, member, cmd) {
    ...
    if(cmd.startsWith("HELP")) {
      ...
      try {
        sections = Trivia.leaderboard.getScoreSections(channel.guild.id);
      ...
      if(isAdmin && typeof author !== "undefined") {
        Trivia.send(author, void 0, `Admin commands: \`${getConfigVal("prefix", channel)}league play\``);
      }
    }
      ...
      try {
        scores = Trivia.leaderboard.readScores(guild.id, userInput, true, doSectionSearch);
      } catch(err) {
      ...
      var scoreStr = Trivia.leaderboard.makeScoreStr(scores, totalParticipants, true);
      var footerText = "";
      ...
      Trivia.send(channel, author, toSend);
    }
      ...
      try {
        scoresWeek = Trivia.leaderboard.readScores(guild.id, "Weekly", true);
      } catch(err) {
        if(err.message !== "Leaderboard is empty") {
          throw err;
        }
      ...
      try {
        scores = Trivia.leaderboard.readScores(guild.id, "Monthly", true);
      } catch(err) {
        if(err.message === "Leaderboard is empty") {
    ...
    // ## Play ## //
      ...
      if(isAdmin) {
        ...
        Trivia.doGame(id, channel, author, 0, category)
        .then((game) => {
        ...
        Trivia.send(channel, author, "Only moderators can use this command. To start a normal game, type `trivia play`");
      }
    }
  }
