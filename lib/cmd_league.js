module.exports = (Trivia, getConfigVal, triviaSend, game, valDatabase, embedCol, leaderboard, doTriviaGame) => {
  ...
  async function leagueParse(id, channel, author, member, cmd) {
    ...
    var scores, userId, toSend, properties;

    // TODO: Section support
    // ## Stats ## //
    if(cmd.startsWith("STATS")) {
      try {
        scores = leaderboard.readScores(guild.id, "Weekly", true);
      } catch(err) {
    ...
    // ## Rank ## //
      ...
      if(typeof scores[userId] === "undefined") {
        ...
        rankStr = `Rank: #${rank} out of ${Object.keys(scores).length}
