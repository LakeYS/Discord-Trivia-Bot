...
class Game extends EventEmitter {
  ...
  async endGame() {
    if(this.inRound) {
      ...
      let finalScoreStr = this.Trivia.gameHandler.leaderboard.makeScoreStr(this.scores, this.totalParticipants);
  ...
  buildRoundEndStr(isEnding) {
    var str = "";
    var scorePrefix;
    var isSingleplayer = Object.keys(this.totalParticipants).length === 1;
    ...
    if(!isEnding && Object.keys(this.correctUsers).length === 0) {
      ...
      var participantId = Object.keys(this.totalParticipants)[0];
      var score = this.scores[participantId];

      if(typeof this.correctUsers[participantId] !== "undefined") {
        str = `Correct, ${this.totalParticipants[participantId]}!\nYour score: ${score}`;
      }
    else {
        str = `Incorrect, ${this.totalParticipants[participantId]}!`;
      }
    }
    else {
      // Standard string
      var participantsToDisplay = isEnding?this.totalParticipants : this.correctUsers;
  ...
  async endRound() {
    ...
    if(Object.keys(this.activeParticipants).length === 0) { 
    ...
    // Finalization
    this.emit("round_end", str, roundTimeout);
