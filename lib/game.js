...
class Game extends EventEmitter {
  ...
  async endGame(endStr) {
    if(this.inRound) {
      ...
      endStr = `Game ended by admin.${finalScoreStr!==""?`\n\n${headerStr}\n`:""}${finalScoreStr}`;
    }
  ...
  submitAnswer(userId, username, isCorrect) {
    ...
    if(isCorrect) {
      ...
      this.scores[userId] += (scoreChange * this.roundMultiplier);
      ...
      if(typeof this.usersCorrect[userId] !== "undefined") {
        ...
        this.roundMultiplier += this.getConfig("score-multiplier-max");
  ...
  buildRoundEndStr(isEnding, isForced) {
    var str = "";
    var scorePrefix;
    var isSingleplayer = Object.keys(this.usersTotal).length === 1;

    if(!this.getConfig("hide-answers")) {
      ...
      str = `${str}${this.formatStr(this.question.answer)}`;
    }

    if(typeof this.answerExtension !== "undefined") {
      str = `${str}\n\n${this.formatStr(this.answerExtension)}`;
    }
    ...
    if(!this.getConfig("disable-score-display")) {
      ...
      if(!isEnding && Object.keys(this.usersCorrect).length === 0) {
        ...
        if(!this.getConfig("use-fixed-rounds") && !this.getConfig("round-end-warnings-disabled") && this.emptyRoundCount >= Math.ceil(this.getConfig("rounds-end-after")/2)) {
        ...
        if(typeof this.usersCorrect[participantId] !== "undefined") {
          str = `${str}Correct, ${this.usersTotal[participantId]}!\nYour score: ${score}`;
        }
      else {
          str = `${str}Incorrect, ${this.usersTotal[participantId]}!`;
        }
      }
      else {
        // Standard string
  ...
  async endRound(isForced) {
    ...
    var str = this.buildRoundEndStr(gameIsEnding, isForced);
