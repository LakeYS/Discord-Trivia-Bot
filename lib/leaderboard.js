...
class Leaderboard {
  ...
  // participantsToDisplay can be game.totalParticipants, or only correct participants for a much shorter list.
  makeScoreStr(scores, participantsToDisplay, largeMode) {
    var scoreArray = [];
    var finalStr = "";
    ...
    scoreArray.forEach((userB) => {
      ...
      if(largeMode) {
        finalStr = `${finalStr}${finalStr!==""?"\n":""}${scoreArray.indexOf(userB)+1}. ${participantsToDisplay[userB]} - ${score.toLocaleString()} points`;
      } else {
        finalStr = `${finalStr}${finalStr!==""?"\n":""}${participantsToDisplay[userB]}: ${score.toLocaleString()}`;
      }
    });
      ...
      for(var section in scores[guildId]) {
        ...
        if(typeof prop.expiredOn === "undefined") {
          ...
          if(section.endsWith("Monthly")) {
            if(typeof prop.expireDate === "undefined" || new Date(prop.expireDate).getDate() !== this.Trivia.getConfigVal("board-monthly-reset-day")) {
              dCurr = new Date();
              dExp = new Date();
              dExp.setMonth(dCurr.getMonth()+1);
              dExp.setDate(this.Trivia.getConfigVal("board-monthly-reset-day")); // Default: 1
              dExp.setMinutes(0);
              dExp.setHours(0);
              dExp.setSeconds(0);
          ...
          if(section.endsWith("Weekly")) {
            dCurr = new Date();
            var weekDay = dCurr.getDate() + (7 - dCurr.getDay()) + this.Trivia.getConfigVal("board-weekly-reset-day");
