  ...
  function writeScores(scores, guildId, sections, teamName) {
    ...
    for(var i in sections) {
      ...
      if(section.endsWith("Monthly")) {
        if(typeof scoresFinal["Properties"].expireDate === "undefined" || new Date(scoresFinal["Properties"].expireDate).getDate() !== getConfigVal("board-monthly-reset-day")) {
          dCurr = new Date();
          dExp = new Date();
      ...
      if(section.endsWith("Weekly")) {
        if(typeof scoresFinal["Properties"].expireDate === "undefined" || new Date(scoresFinal["Properties"].expireDate).getDate() !== getConfigVal("board-weekly-reset-day")) {
          dCurr = new Date();
          dExp = new Date();
          dExp.setMonth(dCurr.getMonth());
          dExp.setDate(dCurr.getDate() + (7 - dCurr.getDay()) + getConfigVal("board-weekly-reset-day")); // Sunday of next week
          dExp.setMinutes(0);
          dExp.setHours(0);
          dExp.setSeconds(0);
          dExp.setMilliseconds(0);
