      ...
      for(var section in scores[guildId]) {
        ...
        if(typeof prop.expiredOn === "undefined") {
          ...
          if(section.endsWith("Monthly")) {
            if(typeof prop.expireDate === "undefined" || new Date(prop.expireDate).getDate() !== this.Trivia.getConfigVal("board_monthly_reset_day")) {
              dCurr = new Date();
              dExp = new Date();
              dExp.setMonth(dCurr.getMonth()+1);
              dExp.setDate(this.Trivia.getConfigVal("board_monthly_reset_day")); // Default: 1
              dExp.setMinutes(0);
              dExp.setHours(0);
              dExp.setSeconds(0);
          ...
          if(section.endsWith("Weekly")) {
            dCurr = new Date();
            var weekDay = dCurr.getDate() + (7 - dCurr.getDay()) + this.Trivia.getConfigVal("board_weekly_reset_day");
