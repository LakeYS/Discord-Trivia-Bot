...
class Game extends EventEmitter {
  ...
  async endRound(importOverride) {
    ...
    if(answer !== this.question.answer && !importOverride) {
      console.warn(`WARNING: Mismatched answers in timeout for this ${this.ID} (${answer}||${this.question.answer})`);
      return;
    }
    ...
    if(this.cancelled) {
      ...
      if(this.emptyRoundCount+1 >= this.getConfig("rounds-end-after")) {
        ...
        if(!this.getConfig("round-end-warnings-disabled") && this.emptyRoundCount >= Math.ceil(this.getConfig("rounds-end-after")/2)) {
          ...
          thisFooter += `Game will end in ${roundEndCount} round${roundEndCount===1?"":"s"} if there is no activity.`;
        }
      }
    } else {
    ...
    if((thisEndedMsg === "" || this.getConfig("disable-score-display")) && !this.getConfig("full-score-display") ) {
      var truncateList = 0;
    ...
    // Finalization
    this.emit("round_end", `${answerStr}${correctUsersStr}${thisEndedMsg}${thisFooter}`, doAutoEnd, roundTimeout);
